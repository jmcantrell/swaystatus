...
class TickerHarness:
    def __init__(self, test_case: TestCase) -> None:
        ...
        patcher = patch.object(self.ticker._next, "wait", side_effect=next_wait_side_effect)
        self.next_wait_mock = patcher.start()
        ...
        patcher = patch.object(self.ticker._next, "set", side_effect=next_set_side_effect)
        self.next_set_mock = patcher.start()
        test_case.addCleanup(patcher.stop)
