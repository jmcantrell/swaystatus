...
class TestEnvironPath(TestEnviron):
    def test_set(self) -> None:
        """A path is returned if the environment variable is set."""
        self.set_env("foo", "file")
        assert environ_path("foo") == Path("file")

    def test_unset(self) -> None:
        """Nothing is returned if the environment variable is not set."""
        self.del_env("foo")
...
class TestEnvironPaths(TestEnviron):
    def test_set(self) -> None:
        """A list of paths is returned if the environment variable is set."""
        self.set_env("foo", "dir1:dir2:dir3")
        assert environ_paths("foo") == [Path("dir1"), Path("dir2"), Path("dir3")]

    def test_unset(self) -> None:
        """An empty list is returned if an environment variable is not set."""
        self.del_env("foo")
...
class TestEnvironAlter(TestEnviron):
    def test_set(self) -> None:
        """Environment variables can be set in bulk."""
        self.del_env("foo")
        self.del_env("bar")
    ...
    def test_unset(self) -> None:
        """Environment variables can be unset in bulk."""
        self.set_env("foo", "a")
        self.set_env("bar", "b")
...
class TestEnvironUpdate(TestEnviron):
    def test_set(self) -> None:
        """Keyword arguments are set in the environment."""
        self.del_env("foo")
    ...
    def test_unset(self) -> None:
        """Keyword arguments set to None values will be unset in the environment."""
    ...
    def test_only(self) -> None:
        """Only the given changes are reverted."""
        self.del_env("foo")
